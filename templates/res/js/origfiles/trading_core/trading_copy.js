...
var substr = ['btc', 'eth', 'ltc', 'trx'];
$.each(substr , function(index, val) { 
  //console.log(val)


	ccc = new WebSocket('wss://stream.binance.com:9443/ws/'+val+'usdt@kline_1m');
	  ccc.onopen = function () {
    };
    ccc.onmessage = function (onmessage) {
		
		let resp_socket = JSON.parse(onmessage.data);
		items[val] = resp_socket;
		//console.log(resp_socket);
		//items.push({val: val, resp_socket: resp_socket});
      ...
      if(buy_amount == "") {
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            if(response == '1') {
                $("#ordersConOne").load("ajax/ajax?action=GET_OPEN_ORDERS"); 
    ...
    setInterval(function() {
      $.ajax({
        ...
        success: function(response) {
          ...
          $("#ordersConOne").load("ajax/ajax?action=GET_OPEN_ORDERS"); 
          $("#ordersConTwo").load("ajax/ajax?action=GET_HISTORY_ORDERS"); 
        }
      })
    }, 10000);
    ///////////////////////////////-END-///////////////////////////////
