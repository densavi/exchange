...
// Отправляет информацию по всем парам раз в секунду
//ccc = new WebSocket('wss://stream.binance.com:9443/ws/!ticker@arr');
...
// setTimeout(function(){
  ...
  // }, 2000);
	...
	// ccc = new WebSocket('wss://stream.binance.com:9443/ws/' + symbol.toLowerCase() + '@ticker');
	
	  // ccc.onopen = function () {
    // };
    // ccc.onmessage = function (onmessage) {
		
		// let resp_socket = JSON.parse(onmessage.data);
	
		// console.log(resp_socket)
		
		// let pair = resp_socket['s']
		// let price = resp_socket['b']
		// let price_change_percent = resp_socket['P']
		
		// $.ajax({
			// url: "/ajax/write_price",
			// type: "POST",
			// data: {
...
$.ajax({
  url: "/ajax/CHECK_TRADING_CURRENCY_PRICE",
  type: "POST",
  data: {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if(response == 'false') {
                    ...
                    high = reloadCurrencyPrice(get_percent(resp_socket.k.h), currency_now_price)
                    low = reloadCurrencyPrice(get_percent(resp_socket.k.l), currency_now_price)
                    close = reloadCurrencyPrice(get_percent(resp_socket.k.c), currency_now_price)
                    ...
                    if(his_edited == 'true') {
                        ...
                        open = reloadCurrencyPrice(get_percent(resp_socket.k.o), currency_now_price)
                    }
                    ...
                    $('html head').find('title').text("$" + addCommas(fixed_price) + " " + new_symbol + "/USDT - Buy and sell Bitcoin and Ethereum");
...
link.forEach(element => {
    ...
    $("#btn_buy").on("click", function() {
      ...
      if(buy_amount == "") {
        ...
        $.ajax({
          url: "ajax/BUY_CREATE_OPEN_ORDERS",
          type: "POST",
          data: {
          ...
          success: function(response) {
            ...
            if(response == '1') {
                $("#ordersConOne").load("ajax/GET_OPEN_ORDERS"); 
      ...
      var sell_fee = $("#sell_fee").html();
        ...
        if(pair_price == 0) {
          ...
          if(sell_amount == "") {
            ...
            noti("Processing..", 'info');
            $.ajax({
              url: "ajax/SELL_CREATE_OPEN_ORDERS",
              type: "POST",
              data: {
                pair_price: pair_price,
                pair_from: pair_one,
    ...
    setInterval(function() {
      $.ajax({
        ...
        success: function(response) {
          ...
          $("#ordersConOne").load("ajax/GET_OPEN_ORDERS"); 
          $("#ordersConTwo").load("ajax/GET_HISTORY_ORDERS"); 
        }
      })
    }, 10000);
    ///////////////////////////////-END-///////////////////////////////
    ...
    $.ajax({
        ...
        success: function(response) {
          ...
          for(var i=0; i < 19; i++) {
            ...
            red_block = red_block + '<div class="order__item" id="red_tr_'+new_tr_block_id+'"> <div class="order__item-fill" style="width: '+random_number()+'%"></div> <div class="order__item-price">'+addCommas(red_block_form)+'</div> <div class="order__item-amount">'+two_amount_red+'</div> <div class="order__item-total">'+addCommas(two_price_red.toFixed(2))+'</div> </div>';

            // green order book


            green_block = green_block + '<div class="order__item" id="green_tr_'+new_tr_block_id+'"><div class="order__item-fill" style="width: '+random_number()+'%"></div> <div class="order__item-price">'+addCommas(green_block_form)+'</div> <div class="order__item-amount">'+two_amount_green+'</div> <div class="order__item-total">'+addCommas(two_price_green.toFixed(2))+'</div> </div>';
          ...
          setInterval(function() {
            ...
            if(json_green_block_form > 10) {
              ...
              timer_red_block = timer_red_block + '<div class="order__item" id="red_tr_'+new_tr_block_id+'"> <div class="order__item-fill" style="width: '+random_number()+'%"></div> <div class="order__item-price">'+addCommas(json_red_block_form)+'</div> <div class="order__item-amount">'+two_amount_red+'</div> <div class="order__item-total">'+addCommas(two_price_red.toFixed(2))+'</div> </div>';

              // green order book


              timer_green_block = timer_green_block + '<div class="order__item" id="green_tr_'+new_tr_block_id+'"><div class="order__item-fill" style="width: '+random_number()+'%"></div> <div class="order__item-price">'+addCommas(json_green_block_form)+'</div> <div class="order__item-amount">'+two_amount_green+'</div> <div class="order__item-total">'+addCommas(two_price_green.toFixed(2))+'</div> </div>';
...
setInterval(function() {
  ...
  if(sign_in == 'true') {
    setInterval(function() {
      $.ajax({
        url: "ajax/GET_TRADING_BALANCE_PANEL",
        type: "POST",
        data: {
  ...
  function disabledBuySelBtn() {
    noti("New users cannot open limit orders and stop buys", "warning");
    noti("At the moment you can open a market order", "info")
  }
