...
class GM {
   ...
   */
	...
	public static function js_ren($arr){
		...
		GM::getConfigIni();
		foreach ($arr as $row){
			$str .= '<script src="'.URL::getServerName().'/spanel/templates/res/js/'.(DEVELOPMENT_FOLDER ? DEVELOPMENT_FOLDER.'/' : null  ).''.$row.''.(NEW_HASH ? '?hash='.time() : null  ).'"></script>'."\n";
		}
	...
	public static function css_ren($arr){
		...
		foreach ($arr as $row){
			$str .= '<link rel="stylesheet" href="'.URL::getServerName().'/spanel/templates/res/css/'.$row.''.(NEW_HASH ? '?hash='.time() : null  ).'">'."\n";
		}
